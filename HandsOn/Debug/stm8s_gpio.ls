 ...
 918  0008 1e03          	ldw	x,(OFST+1,sp)
 919  000a e601          	ld	a,(1,x)
 920  000c 5f            	clrw	x
 921  000d 97            	ld	xl,a
 922  000e 01            	rrwa	x,a
 923  000f 1402          	and	a,(OFST+0,sp)
 924  0011 01            	rrwa	x,a
 925  0012 1401          	and	a,(OFST-1,sp)
 926  0014 01            	rrwa	x,a
 929  0015 5b04          	addw	sp,#4
 930  0017 81            	ret
1008                     ; 225 void GPIO_ExternalPullUpConfig(GPIO_TypeDef* GPIOx, GPIO_Pin_TypeDef GPIO_Pin, FunctionalState NewState)
1008                     ; 226 {
1009                     .text:	section	.text,new
1010  0000               _GPIO_ExternalPullUpConfig:
1012  0000 89            	pushw	x
1013       00000000      OFST:	set	0
1016                     ; 228   assert_param(IS_GPIO_PIN_OK(GPIO_Pin));
1018                     ; 229   assert_param(IS_FUNCTIONALSTATE_OK(NewState));
1020                     ; 231   if (NewState != DISABLE) /* External Pull-Up Set*/
1022  0001 1e07          	ldw	x,(OFST+7,sp)
1023  0003 270a          	jreq	L374
1024                     ; 233     GPIOx->CR1 |= (uint8_t)GPIO_Pin;
1026  0005 1e01          	ldw	x,(OFST+1,sp)
1027  0007 e603          	ld	a,(3,x)
1028  0009 1a06          	or	a,(OFST+6,sp)
1029  000b e703          	ld	(3,x),a
1031  000d 2009          	jra	L574
1032  000f               L374:
1033                     ; 236     GPIOx->CR1 &= (uint8_t)(~(GPIO_Pin));
1035  000f 1e01          	ldw	x,(OFST+1,sp)
1036  0011 7b06          	ld	a,(OFST+6,sp)
1037  0013 43            	cpl	a
1038  0014 e403          	and	a,(3,x)
1039  0016 e703          	ld	(3,x),a
1040  0018               L574:
1041                     ; 238 }
1044  0018 85            	popw	x
1045  0019 81            	ret
1058                     	xdef	_GPIO_ExternalPullUpConfig
1059                     	xdef	_GPIO_ReadInputPin
1060                     	xdef	_GPIO_ReadOutputData
1061                     	xdef	_GPIO_ReadInputData
1062                     	xdef	_GPIO_WriteReverse
1063                     	xdef	_GPIO_WriteLow
1064                     	xdef	_GPIO_WriteHigh
1065                     	xdef	_GPIO_Write
1066                     	xdef	_GPIO_Init
1067                     	xdef	_GPIO_DeInit
1086                     	end
