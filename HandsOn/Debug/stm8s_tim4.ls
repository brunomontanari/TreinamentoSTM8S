...
1081  000c 2003          	jra	L105
1082  000e               L774:
1083                     ; 332     bitstatus = RESET;
1085  000e 5f            	clrw	x
1086  000f 1f01          	ldw	(OFST-1,sp),x
1087  0011               L105:
1088                     ; 334   return ((FlagStatus)bitstatus);
1090  0011 1e01          	ldw	x,(OFST-1,sp)
1093  0013 5b02          	addw	sp,#2
1094  0015 81            	ret
1129                     ; 344 void TIM4_ClearFlag(TIM4_FLAG_TypeDef TIM4_FLAG)
1129                     ; 345 {
1130                     .text:	section	.text,new
1131  0000               _TIM4_ClearFlag:
1135                     ; 347   assert_param(IS_TIM4_GET_FLAG_OK(TIM4_FLAG));
1137                     ; 350   TIM4->SR1 = (uint8_t)(~TIM4_FLAG);
1139  0000 9f            	ld	a,xl
1140  0001 43            	cpl	a
1141  0002 c75344        	ld	21316,a
1142                     ; 351 }
1145  0005 81            	ret
1209                     ; 360 ITStatus TIM4_GetITStatus(TIM4_IT_TypeDef TIM4_IT)
1209                     ; 361 {
1210                     .text:	section	.text,new
1211  0000               _TIM4_GetITStatus:
1213  0000 89            	pushw	x
1214  0001 5204          	subw	sp,#4
1215       00000004      OFST:	set	4
1218                     ; 362   ITStatus bitstatus = RESET;
1220                     ; 364   uint8_t itstatus = 0x0, itenable = 0x0;
1224                     ; 367   assert_param(IS_TIM4_IT_OK(TIM4_IT));
1226                     ; 369   itstatus = (uint8_t)(TIM4->SR1 & (uint8_t)TIM4_IT);
1228  0003 9f            	ld	a,xl
1229  0004 c45344        	and	a,21316
1230  0007 6b01          	ld	(OFST-3,sp),a
1231                     ; 371   itenable = (uint8_t)(TIM4->IER & (uint8_t)TIM4_IT);
1233  0009 c65343        	ld	a,21315
1234  000c 1406          	and	a,(OFST+2,sp)
1235  000e 6b02          	ld	(OFST-2,sp),a
1236                     ; 373   if ((itstatus != (uint8_t)RESET ) && (itenable != (uint8_t)RESET ))
1238  0010 0d01          	tnz	(OFST-3,sp)
1239  0012 270b          	jreq	L355
1241  0014 0d02          	tnz	(OFST-2,sp)
1242  0016 2707          	jreq	L355
1243                     ; 375     bitstatus = (ITStatus)SET;
1245  0018 ae0001        	ldw	x,#1
1246  001b 1f03          	ldw	(OFST-1,sp),x
1248  001d 2003          	jra	L555
1249  001f               L355:
1250                     ; 379     bitstatus = (ITStatus)RESET;
1252  001f 5f            	clrw	x
1253  0020 1f03          	ldw	(OFST-1,sp),x
1254  0022               L555:
1255                     ; 381   return ((ITStatus)bitstatus);
1257  0022 1e03          	ldw	x,(OFST-1,sp)
1260  0024 5b06          	addw	sp,#6
1261  0026 81            	ret
1297                     ; 391 void TIM4_ClearITPendingBit(TIM4_IT_TypeDef TIM4_IT)
1297                     ; 392 {
1298                     .text:	section	.text,new
1299  0000               _TIM4_ClearITPendingBit:
1303                     ; 394   assert_param(IS_TIM4_IT_OK(TIM4_IT));
1305                     ; 397   TIM4->SR1 = (uint8_t)(~TIM4_IT);
1307  0000 9f            	ld	a,xl
1308  0001 43            	cpl	a
1309  0002 c75344        	ld	21316,a
1310                     ; 398 }
1313  0005 81            	ret
1326                     	xdef	_TIM4_ClearITPendingBit
1327                     	xdef	_TIM4_GetITStatus
1328                     	xdef	_TIM4_ClearFlag
1329                     	xdef	_TIM4_GetFlagStatus
1330                     	xdef	_TIM4_GetPrescaler
1331                     	xdef	_TIM4_GetCounter
1332                     	xdef	_TIM4_SetAutoreload
1333                     	xdef	_TIM4_SetCounter
1334                     	xdef	_TIM4_GenerateEvent
1335                     	xdef	_TIM4_ARRPreloadConfig
1336                     	xdef	_TIM4_PrescalerConfig
1337                     	xdef	_TIM4_SelectOnePulseMode
1338                     	xdef	_TIM4_UpdateRequestConfig
1339                     	xdef	_TIM4_UpdateDisableConfig
1340                     	xdef	_TIM4_ITConfig
1341                     	xdef	_TIM4_Cmd
1342                     	xdef	_TIM4_TimeBaseInit
1343                     	xdef	_TIM4_DeInit
1362                     	end
